...
layout: {
    ...
    cy.on('mouseover',function(event){
		...
		if( evtTarget === cy ){
              ...
              if (!$("#showMetabsBox").is(':checked')){
              
                  var node = evtTarget;
                  if (typeof node.data('type') !== 'undefined') {
                    cy.style().selector('node[id = "'+node.data('id')+'"]').css('content', 'data(id)').update();
                  }
              }
		  }
    ...
    cy.on('mouseout',function(event){
		...
		if( evtTarget === cy ){
              ...
              if (!$("#showMetabsBox").is(':checked')){
                  var node = evtTarget;
                  if (typeof node.data('type') !== 'undefined') {
                      cy.style().selector('node[id = "'+node.data('id')+'"]').css('content', '').update();
                  }
              }
		  }
...
function pngExport(){
	...
	w.document.body.innerHTML = '<img src="'+png+'" ></a>';
...
//	console.log(uniqueNumber);
	...
	window.open("analyses/"+uniqueNumber+"/analyses/"+uniqueNumberAnalysis+"/scores.txt");
}
